...
public class CitizenTest {
    ...
    @Test
    public void criteriaExampleSelectNoRestrictions(){
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
//создали строитель объектов запроса
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        fromCitizen:{

            //указываем тип возвращаемых данных
            CriteriaQuery<Citizen> citizenCriteria = cb.createQuery(Citizen.class);

            //указываем откуда брать данные
            Root<Citizen> citizenRoot = citizenCriteria.from(Citizen.class);

            //делаем запрос эквивалент <from Citizen>
            citizenCriteria.select(citizenRoot);
            entityManager.createQuery(citizenCriteria).getResultList().forEach(System.out::println);
        }
        SelectPassportFromSitizen:
        {
            CriteriaQuery<Passport> passportCriteria = cb.createQuery(Passport.class);
            Root<Citizen> citizenPassportRoot = passportCriteria.from(Citizen.class);
            passportCriteria.select(citizenPassportRoot.get("passport"));
            entityManager.createQuery(passportCriteria);
        }
        entityManager.getTransaction().commit();
    ...
    @Test
    public void criteriaExampleSelectRestrictions(){
        //аналог запроса
        //from Passport p where p.citizen.simpleName="Zhukov"
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<Passport> passportCitizenCriteria = cb.createQuery(Passport.class);
        Root<Passport> citizenPassportRoot = passportCitizenCriteria.from(Passport.class);
        passportCitizenCriteria.select(citizenPassportRoot);
        passportCitizenCriteria.where(cb.equal(citizenPassportRoot.get("citizen").get("simpleName"), "Zhukov"));
        entityManager.createQuery(passportCitizenCriteria);

        entityManager.getTransaction().commit();
    }

    @Test
    public void criteriaExampleSelectRestrictionsIN(){
        //аналог запроса
        //from Passport p where p.citizen.simpleName like 'Zh%'
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        entityManager.getTransaction().begin();
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
        ...
        passportLikeCriteria.where(cb.like(likePassportRoot.get("citizen").get("simpleName"), "Zh%"));
        entityManager.createQuery(passportLikeCriteria);
        entityManager.getTransaction().commit();
    }
